...
Сервис поднимется на `http://localhost:8000`, модель автоматически замаунтится из локальной папки `models`. Контейнер перезапускается автоматически при падении (`restart: unless-stopped`).
